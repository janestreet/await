...
(** [on_signal t f] registers [f] to be run when [t] is signalled. It is [true] if [t] was
    unsignalled and the function was registered. It is [false] if [t] was already
    signalled and so the function was not registered.

    Only one callback can be registered with a trigger: [on_signal t f] raises [Failure]
    if [t] already has a registered callback. *)
val on_signal : t @ local -> (unit -> unit) @ once portable -> bool
...
(** [create_with_action action] creates a new trigger that already has the given [action]
    registered with it.
    ...
    Equivalent to:
    {[
      let create_with_action action =
        let trigger = create () in
        let _ : bool = on_signal trigger action in
        trigger
      ;;
    ]} *)
val create_with_action : (unit -> unit) @ once portable -> t
