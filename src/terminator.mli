...
(** [source t] returns the source of the termination token [t] or [None] in case the token
    is not terminatable. *)
val source : t @ local -> Source.t option
...
val with_ : (t @ local -> 'a) @ local once -> 'a
...
(** [with_linked t f] creates a fresh termination token with an associated source and
    ...
    Panics if [t] still has unsignaled attached triggers when [f] finishes. *)
val with_linked : t @ local -> (t @ local -> 'a) @ local once -> 'a
...
(** [with_linked_multi ts f] creates a fresh termination token with an associated source
    ...
    Panics if [t] still has unsignaled attached triggers when [f] finishes. *)
val with_linked_multi : t list @ local -> (t @ local -> 'a) @ local once -> 'a
