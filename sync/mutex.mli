...
module Sync : sig
  ...
  val with_access_poisoning
    ...
    Sync.t @ local
    -> 'k t @ local
    -> f:(Sync.t @ local -> 'k Capsule.Access.t -> 'a @ contended portable unique)
       @ local once portable unyielding
    -> 'a @ contended once portable unique
  ...
  (** [with_key s t ~f] locks [t] and runs [f], providing it a key for ['k] uniquely. If
      [t] is locked, then [with_key] uses [s] to wait until it is unlocked. The [Sync.t]
      is passed to [f] to allow further use of synchronization primitives.
  ...
  (** [with_key_or_cancel s c t ~f] is [Completed (with_key s t ~f)] if [c] is not
      canceled, otherwise it is [Canceled]. *)
